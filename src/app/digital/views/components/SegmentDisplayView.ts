...
import {DigitalPort, SegmentDisplay} from "core/models/types/digital";
...
export class SegmentDisplayView extends ComponentView<SegmentDisplay, DigitalViewInfo> {
    public constructor(info: DigitalViewInfo, obj: SegmentDisplay) {
    ...
    protected override renderComponent({ renderer, selections }: RenderInfo): void {
        ...
        // draw segments
        const segments = this.getSegments(numInputs);
        for (let i = numInputs - 1; i >= 0; --i) { // TODO 9 segments display incorrectly
            const pos = segments[i][0].scale(V(SEGMENT_DISPLAY_WIDTH));
            const on = this.info.sim.getSignal(inputs[i]).valueOf();
    ...
    protected getSegments(inputs: number): Array<[Vector, string]> {
        const segments = Segments[`${inputs}`];
